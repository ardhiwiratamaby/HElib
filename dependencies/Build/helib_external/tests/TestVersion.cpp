...
std::tuple<int, int, int, std::string> readVersionFromCmakeFile()
{
  ...
  const std::string cmakeFilePath = "/home/ardhy/Documents/research/new_project/bgv-comparison/HElib/VERSION";
...
TEST(TestVersion, versionGreaterThanOrEqualTo)
{
  ...
  // Current version
  // clang-format off
  EXPECT_TRUE(helib::version::greaterEquals(2, 2, 0));
  // clang-format on
}
